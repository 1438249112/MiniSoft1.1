...
public class Handle_Mapping_BaseExcel implements IMappingHandler {
	...
	public static IMappingHandler getInstance(Boolean checkFromFields,
		...
		if (!instances.containsKey(key)) {
			instances.put(key, new Handle_Mapping_BaseExcel(checkFromFields,
					checkToFileds));
		}
			...
			for (String tableName : solverTableNames) {
				try {
					...
					for (int i = 0; i < idpFileds.size(); i++) {
						...
						// 4.mapping_in_schema_Fraction
						mapping_in_schema_FractionString += CONSTANTS.in_schema_fraction_template
								.replace("{keyWord}", sfdcFiled);
						// 5.mapping_out_schema_Fraction
					...
					// 4.mapping_in_schema_Fraction
					mapping_in_schema_Fraction.put(objectName,
							mapping_in_schema_FractionString
									.substring(0,
											mapping_in_schema_FractionString
													.length() - 1));
					// 5.mapping_out_schema_Fraction
